...
def get_target_point(params, factor):
...
def get_target_steering_degree(params, factor):
    tx, ty = get_target_point(params, factor)
    car_x = params['x']
    car_y = params['y']
    dx = tx-car_x
    dy = ty-car_y
...
def score_steer_to_point_ahead(params):
    ...
    best_stearing_angle_x15 = get_target_steering_degree(params, 1.5)
    best_stearing_angle_x20 = get_target_steering_degree(params, 2.0)
    error_x15 = (steering_angle - best_stearing_angle_x15) / 60.0  # 60 degree is already really bad
    error_x20 = (steering_angle - best_stearing_angle_x20) / 60.0  # 60 degree is already really bad

    score_x15 = 1.0 - abs(error_x15)
    score_x20 = 1.0 - abs(error_x20)

    return max([score_x15, score_x20, 0.01])  # optimizer is rumored to struggle with negative numbers and numbers too close to zero
