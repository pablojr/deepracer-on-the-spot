...
def score_progress_and_speed(params):
    score = 0.0
    if params['steps'] > 0:
        score = ((params['progress'] / params['steps']) * 100) + (params['speed']**2)
    return score


def reward_function(params):
    return float(score_steer_to_point_ahead(params) + score_progress_and_speed(params))
