...
def score_lap_in_less_steps(params):
    # Incentive for finishing the lap in less steps
    score = 0.01
    steps = params['steps']
    progress = params['progress']
    REWARD_FOR_FASTEST_TIME = 300 # should be adapted to track length and other rewards
    TARGET_STEPS = 215
    if math.isclose(progress, 100.0):
        score = REWARD_FOR_FASTEST_TIME / (steps - TARGET_STEPS)
    return score


def reward_function(params):
    return float(score_steer_to_point_ahead(params) + score_lap_in_less_steps(params))
